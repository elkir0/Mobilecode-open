...
assert.ok(app.includes('api.sendPrompt(config, selectedSession.id, text, selectedSession.directory, activeModel, activeAgentID, pendingAttachments'), 'chat prompts should use selected agent (+ attachments)')
...
assert.ok(/\.sheet-content[\s\S]*?max-height/.test(styles), 'model/details should use a mobile bottom sheet')
assert.ok(/\.model-option-list[\s\S]*?overflow-y/.test(styles), 'searchable model results should scroll in the sheet')
assert.ok(/\.model-option[\s\S]*?min-height:\s*52px/.test(styles), 'model options should be tall enough for two-line labels')
assert.ok(/\.model-option small[\s\S]*?line-height:\s*1\.25/.test(styles), 'model option text should not look vertically squeezed')
