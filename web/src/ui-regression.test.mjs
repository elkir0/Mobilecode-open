...
assert.match(icons, /export const MicIcon/, 'MicIcon should exist in the shared SVG icon set for voice input')
assert.ok(app.includes('Speech') && app.includes('composer-mic'), 'composer should wire the on-device Speech plugin into a mic button')
assert.ok(app.includes('Speech.start') && app.includes('Speech.stop'), 'mic button should start/stop the Speech recognizer')
assert.ok(app.includes('micListening') && app.includes('toggleMic'), 'composer should track a listening state and toggle the mic')
...
assert.ok(app.includes('PUSH_RELAY_URL_STORAGE_KEY'), 'push relay url should persist')
assert.ok(app.includes('PUSH_RELAY_APIKEY_STORAGE_KEY'), 'push relay api key should persist')
...
assert.ok(app.includes('TerminalConsole'), 'terminal console component is wired')
assert.ok(app.includes("t('detail.terminalTitle')"), 'terminal i18n title is wired')
assert.ok(/\.terminal-view[\s\S]*?terminal-output/.test(styles), 'terminal view is styled')

// opencode Question prompts (multiple-choice human-in-the-loop)
assert.ok(app.includes('QuestionPrompt') && app.includes('loadPendingQuestions'), 'App should render QuestionPrompt and poll pending questions')
assert.ok(app.includes('api.replyQuestion') && app.includes('api.rejectQuestion'), 'App should reply/reject opencode questions')
assert.ok(app.includes('inlineQuestions') && app.includes('modalQuestion'), 'questions render inline in detail and as a modal otherwise')
