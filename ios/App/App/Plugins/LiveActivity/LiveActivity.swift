...
public class LiveActivity: CAPPlugin {
  ...
  @objc func startActivity(_ call: CAPPluginCall) {
    ...
    if activities[sessionID] != nil { update(sessionID, title, status, detail); call.resolve(); return }
    let attrs = OpenCodeActivityAttributes(sessionID: sessionID)
    let state = OpenCodeActivityAttributes.ContentState(title: title, status: status, detail: detail)
    do {
  ...
  @objc func updateActivity(_ call: CAPPluginCall) {
    ...
    #if canImport(ActivityKit)
    guard #available(iOS 16.1, *) else { call.resolve(); return }
    update(sessionID, title, status, detail)
    #endif
    call.resolve()
  }
  ...
  #if canImport(ActivityKit)
  @available(iOS 16.1, *)
  private func update(_ sessionID: String, _ title: String, _ status: String, _ detail: String) {
